...
class GraphBuilder:
    ...
    and human-in-the-loop functionality using LangGraph's interrupt capabilities.
    """
    ...
    def human_review(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with the coder_code for the human to review
        """
        print('========== HUMAN REVIEW NODE: Waiting for human review ==========')
        print(f'Current state: {state.model_dump()}')
        ...
        # Print information for console users
        print("\n" + "="*80)
        print("AI REVIEW OF DOCUMENTATION:")
        print("-"*80)
        print(ai_review)
        print("="*80)
        ...
        print("-"*80)
        print(state.coder_code)
        print("-"*80)

        # Get human feedback directly in this node
        while True:
            decision = input("\nDo you approve this documentation? (yes/no): ").strip().lower()
            if decision in ['yes', 'y']:
                human_feedback = {"approved": True, "comments": "Documentation looks good."}
                break
            elif decision in ['no', 'n']:
                human_feedback = {"approved": False, "comments": "Documentation needs improvement."}
                break
            else:
                print("Please enter 'yes' or 'no'.")

        # Return the state with human feedback
        return {
    ...
    def route_based_on_feedback(self, state: WorkflowState) -> Literal['approved', 'rejected', 'end']:
        ...
        """
        # This method should only be called after human feedback has been provided
        # If we're here and there's no human_feedback, it means we're in the middle of an interrupt
        # and this method shouldn't be executing yet

        # Check if human_feedback exists
        if not state.human_feedback:
            # This is a safeguard - we should never reach this point during normal execution
            # because the workflow should be interrupted after human_review
            print("========== ROUTING NODE: No human feedback provided yet ==========")
            print("This is likely an error - the workflow should be interrupted waiting for human feedback")
            # We'll still return a value to avoid crashing, but this should never happen
            return 'rejected'
    ...
    def build_default_graph(self) -> None:
        ...
        4. human_review -> END (direct path for visualization only)
        5. test_generator -> END
        """
